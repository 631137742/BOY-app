...
app.get("/imageList",(req,res)=>{
   ...
   //2:查询
   var list = [
     {id:1,img_url:"http://127.0.0.1:3000/img/banner/mz_001.jpg"},
     {id:2,img_url:"http://127.0.0.1:3000/img/banner/mz_008.jpg"},
     {id:3,img_url:"http://127.0.0.1:3000/img/banner/mz_009.jpg"},
     {id:4,img_url:"http://127.0.0.1:3000/img/banner/mz_010.jpg"},
   ];
   res.send(list); 
});
//功能二:获取新闻列表分页显示
...
app.get("/products",(req,res)=>{
 ...
 sql+=" l.price,p.md";
 sql+=" FROM boy_laptop l,boy_laptop_pic p";
...
app.post("/addcomment",(req,res)=>{
 ...
 //3:创建sql语句
 var sql = "INSERT INTO boy_comment VALUES";
     sql+="(null,?,now(),?)";
 //4:发送sql语句并且返回返回结果
...
app.get("/findProduct",(req,res)=>{
  ...
  //2:sql  SELECT lname,price FROM boy_laptop WHERE //lid=?
  var sql =" SELECT lname,price FROM boy_laptop";
      sql+=" WHERE lid = ?";
  //3:json {code:1,data:[]}
...
app.get("/addcart",(req,res)=>{
  ...
  var sql =" SELECT id FROM boy_cart";
      sql+=" WHERE uid = ? AND pid = ?";
  pool.query(sql,[uid,pid],(err,result)=>{
    if(err)throw err; 
    if(result.length==0){
     var sql = ` INSERT INTO boy_cart`;
     sql+=` VALUES(null,1,${price},${pid},${uid})`;
    }else{
      var sql = ` UPDATE boy_cart`;
...
app.get("/cartlist",(req,res)=>{
 ...
 sql+=" FROM boy_cart c,boy_laptop l";
...
app.get("/delCartItem",(req,res)=>{
  ...
  var id = req.query.id;
  //2:sql  DELETE
  var sql = "DELETE FROM boy_cart WHERE id = ?";
  pool.query(sql,[id],(err,result)=>{
     if(err)throw err;
     if(result.affectedRows > 0){
...
app.get("/removeMItem",(req,res)=>{
  ...
  //2:sql DELETE
  var sql = "DELETE FROM boy_cart ";
      sql+=" WHERE id IN ("+ids+")";
  //3:json http:127.0.0.1:3000/removeMItem?ids=3,8
  pool.query(sql,(err,result)=>{
    if(err)throw err;
    if(result.affectedRows > 0){
      res.send({code:1,msg:"删除成功"})
...
app.get("/imagelist2",(req,res)=>{
  var rows = [
    {code:1,img_url:"http://127.0.0.1:3000/img/banner/mz_009.jpg"},
    {code:2,img_url:"http://127.0.0.1:3000/img/banner/mz_010.jpg"},
    {code:3,img_url:"http://127.0.0.1:3000/img/banner/mz_011.jpg"},
    {code:4,img_url:"http://127.0.0.1:3000/img/banner/mz_012.jpg"},
  ];
  res.send(rows);
})
//3:创建js对象 图片编号 图片地址
...
app.get("/icons",(req,res)=>{
  var rows = [
  {id:1,img_url:"http://127.0.0.1:3000/img/icon/grid-01.png",title:"商品"},
  {id:2,img_url:"http://127.0.0.1:3000/img/icon/grid-02.png",title:"添加商品"},
  {id:3,img_url:"http://127.0.0.1:3000/img/icon/grid-03.png",title:"购物车"},
  {id:4,img_url:"http://127.0.0.1:3000/img/icon/grid-04.png",title:"新闻"},
  {id:5,img_url:"http://127.0.0.1:3000/img/icon/grid-05.png",title:"品牌"},
  {id:6,img_url:"http://127.0.0.1:3000/img/icon/grid-06.png",title:"搜索商品"},
  ];
  res.send(rows);
})
// 小程序 首页店铺
app.get("/news",(req,res)=>{
  var rows = [
    {id:1,img_url:"http://127.0.0.1:3000/img/section/0b9cf390c1df8e0f",title:"林荫树街店"},
    {id:2,img_url:"http://127.0.0.1:3000/img/section/444e8e0f336e1380",title:"BOYLONDON INTERNATIONAL店"},
    {id:3,img_url:"http://127.0.0.1:3000/img/section/6dc7d9e870f38d05",title:"三清店"},
    {id:4,img_url:"http://127.0.0.1:3000/img/section/63db0fbab571fab8",title:"济州神话世界免税店"},
    {id:5,img_url:"http://127.0.0.1:3000/img/section/c3560b0c30e93575",title:"明洞2街店"},
    {id:6,img_url:"http://127.0.0.1:3000/img/section/93a00fbdec850fdb",title:"济州中央店"}
  ];
  res.send(rows);
});
